...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:access="xalan://com.pilotfish.utils.AttributeAndPropertyAccessor" xmlns:dtFormatter="xalan://com.pilotfish.eip.gui.mapper.util.DateTimeFormatter" exclude-result-prefixes="dtFormatter access" version="3.1">
    ...
    <XCSData>
      <ADT_A02>
        <MSH>
          ...
          <MSH.7>
            <xsl:value-of select="dtFormatter:format(ASSIGNMENTTIME,'yyyy-MM-dd hh:mm:ss.S','yyyyMMddhhmmss.SSSS')" />
          </MSH.7>
          <MSH.8 />
          <MSH.9>
            <MSG.1>ADT</MSG.1>
          ...
          <xsl:call-template name="pf_setAttribute">
            ...
            <xsl:with-param name="value" select="concat(ROOTOFFENDERID,LEVEL1CODE)" />
          </xsl:call-template>
        </MSH>
        <EVN>
          <EVN.1>A02</EVN.1>
        ...
        <PID>
          ...
          <PID.6 />
          <PID.7>
            <!--WE DONT HAVE BDATE IN THE INCOMING QUERY RESULTS-->
            <xsl:value-of select="dtFormatter:format(BRITHDATE,'yyyy-MM-dd hh:mm:ss.S','yyyyMMdd')" />
          </PID.7>
          <!--REQUIRED - GENDER-->
          <PID.8>
        ...
        <PV1>
          ...
          <!--REQUIRED - TODO - NEED TO ADD LEVEL2 AND LEVEL3 TO QUERY-->
          <PV1.3>
            <PL.1>
