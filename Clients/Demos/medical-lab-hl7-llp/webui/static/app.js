...
function renderList(el, items) {
  ...
  el.innerHTML = items
    .map(
      (i) =>
        `<article><strong>${i.name}</strong><div class="muted">${i.controlId || ""}</div><pre>${escapeHtml(
          i.preview || ""
        )}</pre></article>`
    )
    .join("");
...
async function refresh() {
  ...
  renderList(document.getElementById("list-received"), data.meditechReceived || []);
}
...
document.getElementById("btn-send").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await getJson("/api/send-llp", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sample }),
    });
    ackOut.textContent = data.ack || "(empty ACK)";
    status.textContent = "LLP send complete";
...
/* tabs + routes (same pattern as other demos) */
