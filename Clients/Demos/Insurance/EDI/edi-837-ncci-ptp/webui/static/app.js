...
function renderCatalog(rows) {
  ...
  if (!el) return;
  if (!rows.length) {
    el.innerHTML = "<p class='muted'>(no PTP catalog rows)</p>";
    return;
  }
  const cols = ["Column1", "Column2", "ModifierIndicator", "Description"];
  el.innerHTML =
...
async function refresh() {
  try {
    const catalog = await getJson("/api/ptp-edits");
  ...
  try {
    ...
    renderFiles("pass-list", "pass-view", results.pass || []);
    renderFiles("kickout-list", "kickout-view", results.kickout || []);
  } catch (_) {
...
async function loadRoutesTab() {
  ...
  if (!routesLoaded) {
    ...
    select.innerHTML = (data.routes || []).map((r) => `<option value="${r.id}">${r.name}</option>`).join("");
    routesLoaded = true;
    select.addEventListener("change", () => {
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      try {
        await getJson("/api/wait-results?contains=PTPCLM001_pass.xml&timeout=90");
...
document.getElementById("reset-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus("Output folders cleared.");
    await refresh();
  } catch (err) {
    setStatus(err.message || String(err), true);
  }
