...
async function refresh() {
  try {
    ...
    renderFiles("qcare-list", "qcare-view", results.qcare || []);
    renderFiles("debug-list", "debug-view", results.debug || []);
  } catch (_) {
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      setStatus(`Dropped ${result.file}. Waiting for QCare…`);
      try {
        await getJson("/api/wait-results?contains=STONE&timeout=90");
        setStatus("QCare record landed.");
      } catch (err) {
