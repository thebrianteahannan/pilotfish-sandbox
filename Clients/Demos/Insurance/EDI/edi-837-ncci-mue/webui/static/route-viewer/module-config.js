...
(function (global) {
  ...
  function describeValue(configKey, raw) {
    ...
    return {
      ...
      display: summary,
      summary,
      // Keep full for tooling; demos only show the readable summary.
      full: ognl.full,
      title: summary,
    };
  }
  ...
  /**
   * Inline diagram cards (config=changed) only show demo-relevant settings.
   * Everything else stays available in the detail panel — not on the node face.
   */
  ...
  /** Never show these on the node face — obscure / ops noise. */
  const INLINE_DENY = new Set([
    "FilenameIsMvsDsn",
    "UsePassiveMode",
    "SSLProtocol",
    "Sftp_SocketTimeout",
    "SerializedTransactionsTag",
    "RetryConnection",
    "RetryDelay",
    "Timeout",
    "CompatModeMoved",
    "Tokenizers",
    "SubFolderIterationTag",
    "HiddenFilesTag",
    "FileSortingMethod",
    "FileSortingDirection",
    "MinDaysSinceFileModified",
    "MaxDaysSinceFileModified",
    "CombineFiles",
    "HeaderLines",
    "SynchResponse",
    "CheckInactivity",
    "ThrottlingMode",
    "ThrottlingMechanism",
    "ThrottlingConcurrentMessages",
    "TransactionLoggingAllowed",
    "TransactionLoggingStoreData",
    "CLIAllowed",
    "IS_TRIGGERABLE_LISTENER",
    "RESTART_ON_ERROR_TAG",
    "StopWhenPollingFailure",
    "FIFOQueueName",
    "FIFOQueueDelay",
    "MAXIMUM_MEMORY_SIZE",
    "FileNameConflictPattern",
    ...
    "KeepConnection",
    "Autocommit",
    "UseJdbcGeneratedKeys",
    "LogMetadata",
    "LogSQL",
    "CacheXSLTToXML",
    "XSLTParameters",
    "SaxonConverterHandling",
    "SaxonConverterEncoding",
    "JDBCProperties",
  ]);

  function isInlineRelevantKey(key) {
    if (!key) return false;
    if (INLINE_DENY.has(key)) return false;
    if (INLINE_ALLOW.has(key)) return true;
    if (INTERESTING_IF_EMPTY.has(key)) return true;
    // Soft patterns for module-specific but still human-meaningful keys
    return /^(Host|Port|User|Password|Polling|Target|FileName|FileExtension|XSLT|Jdbc|Delimiter|Header|PostProcess|ListenerType)/i.test(
      key
    );
  }
  ...
  function getInlineEntries(mod, mode) {
    ...
    if (mode === "changed") {
      return entries.filter((e) => {
        ...
        if (!isInlineRelevantKey(e.key)) return false;
  ...
  function renderInlineValue(described) {
    if (described.kind === "ognl") {
      // Demo cards: readable summary only — hide raw {ognl:…}.
      return `<span class="cfg-v">
        <span class="cfg-ognl-sum">${escapeHtml(truncateInline(described.summary, 120))}</span>
      </span>`;
    }
  ...
  function estimateInlineSize(mod, mode, label) {
    ...
    shown.forEach((e) => {
      ...
      if (described.kind === "ognl") {
        hasOgnl = true;
        const sumLen = (described.summary || "").length;
        lineUnits += Math.min(2, Math.max(1, Math.ceil(sumLen / 56)));
      } else {
        lineUnits += 1;
      }
    });
