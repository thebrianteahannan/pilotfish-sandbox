...
function renderCatalog(rows) {
  ...
  if (!el) return;
  if (!rows.length) {
    el.innerHTML = "<p class='muted'>(no MUE catalog rows)</p>";
    return;
  }
  const cols = ["Cpt", "MaxUnits", "Mai", "Description"];
  el.innerHTML =
...
async function refresh() {
  try {
    const catalog = await getJson("/api/mue-edits");
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      try {
        await getJson("/api/wait-results?contains=MUECLM001_pass.xml&timeout=90");
