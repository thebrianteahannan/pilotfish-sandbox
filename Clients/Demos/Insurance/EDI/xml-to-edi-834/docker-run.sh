#!/usr/bin/env bash
# Run the XML → EDI 834 PilotFish demo (Clients/Demos/xml-to-edi-834).
#
# Usage (from this directory or via repo-root ./docker-run-834.sh):
#   ./docker-run.sh build|start|stop|restart|status|logs|shell|url|demo
#   ./docker-run.sh          # start (build if needed), then follow eip.log
#   ./docker-run.sh demo     # start, drop sample XML, wait for .834 output

set -euo pipefail

DEMO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SANDBOX_ROOT="$(cd "${DEMO_DIR}/../../.." && pwd)"
IMAGE_NAME="${IMAGE_NAME:-pilotfish-eip:23R1}"
CONTAINER_NAME="${CONTAINER_NAME:-pilotfish-eip-834}"
HOST_PORT="${HOST_PORT:-8081}"

EIP_ROOT_DIR="${DEMO_DIR}/eip-root"
DATA_DIR="${DEMO_DIR}/data"
LOGS_DIR="${DEMO_DIR}/logs"
DEBUG_TRACE_DIR="${DEMO_DIR}/debug-trace"
...
cmd_build() {
  echo "Building ${IMAGE_NAME} (base image; this demo mounts eip-root at runtime) ..."
  docker build -t "${IMAGE_NAME}" "${SANDBOX_ROOT}"
  echo "Build complete."
}
