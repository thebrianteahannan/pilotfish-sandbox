...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="payerIdSafe" select="if (normalize-space(($r/PayerId)[1])!='') then normalize-space(($r/PayerId)[1]) else 'MOCKPAYER'"/>
  <xsl:variable name="payerNameSafe" select="if (normalize-space(($r/PayerName)[1])!='') then normalize-space(($r/PayerName)[1]) else 'MOCK PAYER'"/>
  <xsl:variable name="npiSafe" select="if (normalize-space(($r/ProviderNpi)[1])!='') then normalize-space(($r/ProviderNpi)[1]) else '1234567893'"/>
  <xsl:variable name="providerSafe" select="if (normalize-space(($r/ProviderName)[1])!='') then normalize-space(($r/ProviderName)[1]) else 'PILOTFISH DEMO CLINIC'"/>
  <xsl:variable name="svcSafe" select="if (normalize-space(($r/ServiceTypeCode)[1])!='') then normalize-space(($r/ServiceTypeCode)[1]) else '30'"/>
  <xsl:variable name="traceSafe" select="if (normalize-space(($r/TraceNumber)[1])!='') then normalize-space(($r/TraceNumber)[1]) else 'TRACE001'"/>
  <xsl:variable name="dateSafe" select="if (normalize-space(($r/ServiceDate)[1])!='') then normalize-space(($r/ServiceDate)[1]) else '20260813'"/>
  <xsl:variable name="isa13" select="format-number(number(substring(translate($traceSafe,'ABCDEFGHIJKLMNOPQRSTUVWXYZ-',''),1,9)), '000000000')"/>
  <xsl:variable name="ctrl" select="format-number(number(substring(translate($traceSafe,'ABCDEFGHIJKLMNOPQRSTUVWXYZ-',''),1,4)), '0001')"/>
  <xsl:variable name="yymmdd" select="substring($dateSafe, 3, 6)"/>
  <xsl:template match="/">
    <XCSData>
      <Interchange AuthorizationQual="00" AuthorizationInfo="          "
        ...
        <Group GroupType="HS" ApplSender="CLINICDEMO" ApplReceiver="{$payerIdSafe}"
          ...
          <Transaction DocType="270" ControlNumber="{$ctrl}" StandardVersion="005010X279A1">
            <BHT>
              <BHT01>0022</BHT01><BHT02>13</BHT02>
            ...
            <Loop_2000C>
              ...
              <Loop_2100C>
                ...
                <DTP>
                  <DTP01>291</DTP01><DTP02>D8</DTP02>
                  <DTP03><xsl:value-of select="$dateSafe"/></DTP03>
                </DTP>
                <EQ><EQ01><xsl:value-of select="$svcSafe"/></EQ01></EQ>
              </Loop_2100C>
            </Loop_2000C>
          </Transaction>
        </Group>
