...
(function () {
  ...
  function pipelineNodes(status) {
    const p = (status && status.pipeline) || {};
    const named = (slot, fallbackRole, fallbackName) => ({
      k: slot === "in" ? "in" : slot === "mid" ? "mid" : "out",
      role: (p[slot] && p[slot].role) || fallbackRole,
      name: (p[slot] && p[slot].name) || fallbackName,
    });
    if (p.in || p.mid || p.out) {
      return [
        named("in", "Listener", "Listener"),
        named("mid", "Processors", "Processors"),
        named("out", "Transport", "Transport"),
      ];
    }
  ...
  function itemHint(item, extras, state, status) {
    if (item.id === "design" && state === "now") {
  ...
  function routesToolbar() {
    return document.querySelector(".routes-panel > .row-head");
  }

  function setLiveConstruction(on) {
    document.body.classList.toggle("pf-live-construction", !!on);
    const toolbar = routesToolbar();
  ...
  function paint(status, extras) {
    ...
    const nodes = pipelineNodes(status).map((n, i) => {
      const allReady = published || stackSeenUp;
      const choosing = phase === "design";
      const building = !allReady && nowIdx >= 0 && i === Math.min(nowIdx, 2);
      const ready = allReady || (nowIdx >= 0 && i < Math.min(nowIdx, 2));
      let state = "Queued";
      if (allReady || ready) state = "Ready";
      else if (choosing) state = n.name && n.name !== "Listener" && n.name !== "Processors" && n.name !== "Transport" ? "Chosen" : "Choosing";
      else if (building) state = "Building";
      return {
        ...n,
        state,
        lit: allReady || ready || building || choosing,
        pulse: choosing || building,
      };
    });
    ...
    items.forEach((s) => {
      ...
      setText(li.querySelector("p"), itemHint(s, extras, s.state, status));
    });
