...
function renderAr(rows) {
  ...
  if (!el) return;
  if (!rows.length) {
    el.innerHTML = "<p class='muted'>(no AuthRequest rows)</p>";
    return;
  }
  const cols = ["AuthTraceNumber", "PatientLastName", "PatientFirstName", "ProcedureCode", "Status"];
  el.innerHTML =
...
async function refresh() {
  try {
    const ar = await getJson("/api/auth-requests");
    renderAr(ar.rows || []);
  } catch (err) {
    renderAr([]);
  ...
  try {
    ...
    renderFiles("approved-list", "approved-view", results.approved || []);
    renderFiles("denied-list", "denied-view", results.denied || []);
    renderFiles("incomplete-list", "incomplete-view", results.incomplete || []);
    renderFiles("pended-list", "pended-view", results.pended || []);
    renderFiles("responses-list", "responses-view", results.responses || []);
    renderFiles("ehr-list", "ehr-view", results.ehr || []);
  } catch (_) {
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      try {
        await getJson("/api/wait-results?contains=PACOMPLETE01&timeout=90");
...
document.getElementById("reset-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus("AuthRequest reset to OPEN. Output folders cleared.");
    await refresh();
  } catch (err) {
    setStatus(err.message || String(err), true);
  }
