...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:text>","memberId":"</xsl:text><xsl:value-of select="pf:jesc($memberId)"/>
    <xsl:text>","lastName":"</xsl:text><xsl:value-of select="pf:jesc($last)"/>
    <xsl:text>","firstName":"</xsl:text><xsl:value-of select="pf:jesc($first)"/>
    ...
    <xsl:for-each select="$ebSegs">
      ...
      <xsl:text>{"eb01":"</xsl:text><xsl:value-of select="pf:jesc($c)"/>
    ...
    <xsl:text>],"parseMode":"structural-x12-xslt","message":"</xsl:text>
    <xsl:choose>
      ...
      <xsl:otherwise><xsl:text>Parsed 271 but found neither AAA nor EB.</xsl:text></xsl:otherwise>
    </xsl:choose>
    <xsl:text>"}</xsl:text>
  </xsl:template>
</xsl:stylesheet>
