...
(function () {
  ...
  const replayStep = (params.get("replayStep") || "").trim();
  const BASE = replayStep
    ? `/api/build-replay/steps/${encodeURIComponent(replayStep)}`
    : routeKey
  ...
  const focusLabel = (params.get("focusLabel") || "").trim();
  const focusNodeId = (params.get("focusNode") || "").trim();
  ...
  function applyDocsWorldSize(worldW, worldH) {
    ...
    if (state.selectedId) {
      requestAnimationFrame(() => {
        const nodeEl = el.nodesLayer.querySelector(`.route-node[data-id="${state.selectedId}"]`);
        if (nodeEl) {
  ...
  function centerOn(n) {
    if (docsMode) {
      const nodeEl = el.nodesLayer.querySelector(`.route-node[data-id="${n.id}"]`);
      if (nodeEl) {
        nodeEl.scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
      }
      return;
  ...
  async function boot() {
    ...
    if (!replayStep) {
      ...
      window.ROUTE_VIEWER_BASE = base;
      xmlUrl = `${base}/route.v2.xml`;
      groupsUrl = `${base}/diagram-groups.json`;
    }

    el.status.textContent = replayStep
      ? `Loading replay step ${replayStep}…`
      : "Loading route.v2.xml…";
    try {
      ...
      if (!route.nodes.length) {
        fitToView();
        el.status.textContent = replayStep ? "Empty canvas — next module coming…" : "Ready (empty route)";
        document.documentElement.setAttribute("data-ready", "1");
        return;
      }
      fitToView();
      // Construction theater: highlight + center the newly added module
      let focus = null;
      if (focusNodeId) focus = route.nodes.find((n) => n.id === focusNodeId);
      if (!focus && focusLabel) {
        focus = route.nodes.find((n) => (n.label || "") === focusLabel)
          || route.nodes.find((n) => (n.label || "").includes(focusLabel));
      }
      if (!focus && (focusLabel || focusNodeId || replayStep)) {
        focus = route.nodes[route.nodes.length - 1];
      }
      if (focus) {
        selectNode(focus.id);
        const nodeEl = el.nodesLayer.querySelector(`.route-node[data-id="${focus.id}"]`);
        if (nodeEl) {
          nodeEl.classList.add("is-new");
          setTimeout(() => nodeEl.classList.remove("is-new"), 2800);
        }
        if (!docsMode) {
          state.scale = Math.max(state.scale, 0.85);
        }
        centerOn(focus);
        el.status.textContent = `Focused: ${focus.label}`;
      } else {
