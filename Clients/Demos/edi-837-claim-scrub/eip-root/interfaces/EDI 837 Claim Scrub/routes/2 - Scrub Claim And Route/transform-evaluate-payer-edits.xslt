...
<!-- Evaluate claim vs synthetic payer edit profile before clearinghouse leave. -->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- Synthetic payer profile table (mirrors dbo.PayerEditRules seed). -->
  <xsl:variable name="requireRef"
                select="if ($payerId = '66783JJT') then true() else false()"/>
        ...
        <xsl:choose>
          <xsl:when test="$kick">
            ...
            <xsl:value-of select="string-join((
                if ($missingRef) then concat($payerName, ' requires a referring provider NPI before clearinghouse submit.') else (),
                if ($invalidPos) then concat('Place of Service ', $pos, ' is not allowed for ', $payerName,
                  ' (allowed: ', string-join($allowedPos, ', '), ').') else ()
              ), ' ')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>Claim </xsl:text>
            <xsl:value-of select="$claimNumber"/>
            <xsl:text> passed payer edits; continue to 837 + SNIP outbound.</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </HumanSummary>
    </ClaimScrubDecision>
