...
async function refresh() {
  const [downloads, archive, remote] = await Promise.all([
    getJson("/api/downloads"),
    getJson("/api/archive"),
    getJson("/api/remote"),
  ]);
  renderFiles("download-list", "download-view", downloads.files || []);
  renderFiles("archive-list", "archive-view", archive.files || []);
  renderFiles("remote-list", "remote-view", remote.files || []);
}
...
async function loadSamples() {
  ...
  select.addEventListener("change", () => {
    ...
    document.getElementById("control").value = match ? match.content : "";
  });
  if (data.files && data.files[0]) {
    select.value = data.files[0].name;
    document.getElementById("control").value = data.files[0].content;
  }
}
...
document.getElementById("inject-form").addEventListener("submit", async (e) => {
  ...
  try {
    const result = await getJson("/api/inject", {
      ...
      body: JSON.stringify({ sample, control }),
    });
    const remoteName = result.remoteName || "download";
    setStatus(`Dropped ${result.file}. Waiting for ${remoteName}…`);
    const waited = await getJson(
      `/api/wait-download?file=${encodeURIComponent(remoteName)}&timeout=90`
    );
    setStatus(`Downloaded ${waited.file?.name || remoteName}`);
    await refresh();
  } catch (err) {
