...
TYPE_BY_CLASS = {
    ...
    "com.pilotfish.eip.modules.other.RegExIntoTransactionAttributeProcessor": "RegEx to Attribute",
    ...
    "com.pilotfish.eip.modules.http.HttpResponseCodeProcessor": "Http Response Status Code",
...
def main():
    iface = Path(
        ...
        "fhir-patient-exchange/eip-root/interfaces/FHIR Patient Exchange"
    )
    formats = iface / "formats"
    # Also convert docker twin under pilotfish/demo-eip-root
    targets = [iface]
    demo_root = Path(
        "/Users/brianhannan/Documents/PilotFish Sandbox/Clients/Demos/"
        "fhir-patient-exchange/pilotfish/demo-eip-root"
    )
    if (demo_root / "routes").is_dir():
        targets.append(demo_root)

    for root in targets:
        formats_dir = root / "formats" if (root / "formats").is_dir() else formats
        for route in sorted((root / "routes").iterdir()):
            if not (route / "route.xml").is_file():
                continue
            print("Converting", root.name, "→", route.name)
