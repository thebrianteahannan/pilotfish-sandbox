...
function renderMessages(messages) {
  ...
  const rows = messages
    .map(
      (m) => `<tr>
      <td>${m.MessageId}</td>
      <td>${m.HospitalCode || ""}</td>
      <td>${m.MessageType || ""}^${m.TriggerEvent || ""}</td>
      <td>${m.PatientId || ""}</td>
      <td>${m.PatientName || ""}</td>
  ...
  wrap.innerHTML = `<table>
    <thead><tr><th>Id</th><th>Hospital</th><th>Type</th><th>Patient</th><th>Name</th><th>Status</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}

async function refresh() {
  const [messages, ch, val, ko] = await Promise.all([
    getJson("/api/messages"),
    getJson("/api/clearinghouse"),
    getJson("/api/validation"),
    getJson("/api/kickout"),
  ]);
  renderMessages(messages.messages || []);
  renderFiles("ch-list", "ch-view", ch.files || []);
...
async function loadSamples() {
  ...
  select.addEventListener("change", () => {
    ...
    document.getElementById("hl7").value = hit ? hit.content : "";
    for (const code of ["HOSP-01", "HOSP-02", "HOSP-03"]) {
      if (select.value.startsWith(code)) {
        document.getElementById("hospitalCode").value = code;
        break;
      }
    }
  });
...
document.getElementById("inject-form").addEventListener("submit", async (ev) => {
  ...
  try {
    ...
    const waited = await getJson(`/api/wait-message?controlId=${encodeURIComponent(created.meta.controlId)}&timeout=60`);
    if (waited.message) setStatus(`Inserted message #${waited.message.MessageId}`);
