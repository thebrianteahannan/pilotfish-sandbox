#!/usr/bin/env python3
"""Minimal MLLP sink that pretends to be MEDITECH: log ORUs and return AA ACK."""
...
def build_aa(msh_line: str) -> bytes:
    ...
    # Swap sending/receiving app+facility for ACK
    ack = (
        f"MSH|^~\\&|{fields[4]}|{fields[5]}|{fields[2]}|{fields[3]}|{ts}||ACK^R01|{ctrl}-ACK|P|{version}\r"
        f"MSA|AA|{ctrl}|MEDITECH mock accepted"
    )
...
class Handler(socketserver.BaseRequestHandler):
    def handle(self) -> None:
        ...
        name = f"MEDITECH_{dt.datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.hl7"
        (OUT / name).write_text(text.replace("\r", "\n"), encoding="utf-8")
        print(f"Received {len(text)} bytes -> {name}", flush=True)
...
if __name__ == "__main__":
    print(f"MEDITECH mock MLLP listening on {PORT}", flush=True)
