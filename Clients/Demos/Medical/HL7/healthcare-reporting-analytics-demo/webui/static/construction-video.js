(function () {
  ...
  const sectionBtn = document.getElementById("pf-rerecord-section-btn");
  const sectionRow = document.getElementById("pf-video-section-row");
  const sectionSel = document.getElementById("pf-video-section");
  ...
  const playerWrap = document.getElementById("pf-video-player-wrap");
  if (!btn) return;

  function liveJob(job) {
    if (!job || job.status !== "running") return null;
    const stamp = Date.parse(job.started_at || job.updated_at || "") || 0;
    if (!stamp) return null;
    if (Date.now() - stamp > 20 * 60 * 1000) return null;
    return job;
  }
  ...
  function paintProgress(job, running, ready, celebrate) {
    ...
    if (justDone && !running) {
      ...
      if (bannerMsg) bannerMsg.textContent = "Click Watch construction video on the Video tab.";
  ...
  function apply(data) {
    let job = liveJob((data && data.job) || null);
    ...
    if (playerWrap) playerWrap.hidden = !ready;
    ...
    if (sectionRow && sectionSel) {
      sectionRow.hidden = !ready || sections.length === 0;
      if (sectionSel.options.length !== sections.length) {
        const keep = sectionSel.value;
        sectionSel.innerHTML = sections
          .map((row) => {
            const id = (row && row.id) || "";
            const title = (row && row.title) || id;
            return id ? `<option value="${id}">${title}</option>` : "";
          })
          .join("");
        if (keep && [...sectionSel.options].some((opt) => opt.value === keep)) {
          sectionSel.value = keep;
        }
      }
    }
    if (sectionBtn) sectionBtn.disabled = running || !ready || sections.length === 0;
  ...
  async function startExport(section) {
    btn.disabled = true;
    if (sectionBtn) sectionBtn.disabled = true;
    btn.textContent = section ? "Updating section…" : "Creating video…";
    setStatus("Starting exporter…");
    lastJob = {
      status: "running",
      phase: "starting",
      message: section ? `Re-recording ${section}…` : "Starting exporter…",
      started_at: new Date().toISOString(),
      log: [{ text: section ? `Re-record section ${section}` : "Clicked Create construction video" }],
    };
    wasRunning = true;
  ...
  if (sectionBtn) {
    sectionBtn.addEventListener("click", () => startExport((sectionSel && sectionSel.value) || ""));
  }
