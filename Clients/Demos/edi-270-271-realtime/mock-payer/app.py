"""Mock X12 eligibility payer — accepts 270, returns AAA or success 271."""
...
def wrap_271(body_segments: list[str], trace: str, control: str = "0001") -> str:
    today = time.strftime("%Y%m%d")
    yymmdd = time.strftime("%y%m%d")
    hhmm = time.strftime("%H%M")
    se_count = len(body_segments) + 2  # ST + SE
    ...
    # Fix SE count: ST + body + SE
    se_count = len(body_segments) + 2
    segments[-3] = f"SE*{se_count}*{control}~"
...
def build_aaa_271(info: dict, code: str = "72") -> str:
    ...
    return wrap_271(body, info["trace"])
...
def eligibility_270():
    ...
    if mid in {"FAIL001", "BADID", ""}:
        code = "72"
        out = build_aaa_271(info, code=code)
        outcome = "aaa"
