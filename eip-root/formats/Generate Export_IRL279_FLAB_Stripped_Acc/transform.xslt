	...
	<xsl:template match="/XCSData">
		...
		<XCSExcelBook>
			<XCSExcelSheet name="Export_IRL279_FLAB_Stripped_Acc">
				...
				<xsl:for-each select="Import/Group[@stripped = 'true']/PatientDemographics[admLocation = 'F.LAB'][string-length(Filler2) &gt; 0]">
					<xsl:variable name="admLocation" select="admLocation"/>
					<xsl:variable name="admName" select="admName"/>
					<xsl:variable name="Filler2" select="Filler2"/>
					<xsl:if test="count($stripLocations[LOCATION = $admLocation and LOCATION2 = $Filler2]) != 0">
						<XCSExcelRow>
							<admAcctNum>
