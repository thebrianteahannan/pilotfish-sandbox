...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ns1="http://pilotfish.sqlxml" version="3.1">
  <xsl:template match="XCSExcelBook">
    <ns1:SQLXML>
      ...
      <xsl:variable name="NumSplits" select="count(//SPLIT_CODE)" />
      ...
      <xsl:for-each select="XCSExcelSheet/XCSExcelRow[string-length(MUE_EDITS_CPT) &gt; 0 and string-length(MUE_EDITS_MAX_VALUE_PER_LINE) &gt; 0 and string-length(MUE_EDITS_CDM) &gt; 0]">
        <ns1:Insert>
          <MUE_EDITS>
            <SOFTWARE_ID>
              <xsl:value-of select="$SoftwareID" />
            ...
            <CPT>
              <xsl:value-of select="MUE_EDITS_CPT" />
            </CPT>
            <MAX_VALUE_PER_LINE>
              <xsl:value-of select="MUE_EDITS_MAX_VALUE_PER_LINE" />
            </MAX_VALUE_PER_LINE>
            <CDM>
              <xsl:value-of select="MUE_EDITS_CDM" />
            </CDM>
          </MUE_EDITS>
        </ns1:Insert>
      </xsl:for-each>
      <xsl:for-each select="XCSExcelSheet/XCSExcelRow[string-length(CUSTOM_STRIPPING_RULES) &gt; 0]">
        <ns1:Insert>
          <STRIPPING_RULES>
            <PARTITION>
              <xsl:value-of select="$Partition" />
            ...
            <RULE_NAME>
              <xsl:value-of select="CUSTOM_STRIPPING_RULES" />
            </RULE_NAME>
          </STRIPPING_RULES>
        </ns1:Insert>
      </xsl:for-each>
      <xsl:for-each select="XCSExcelSheet/XCSExcelRow[string-length(CUSTOM_TWEAKING_RULES) &gt; 0]">
        <ns1:Insert>
          <TWEAKING_RULES>
            <PARTITION>
              <xsl:value-of select="$Partition" />
            ...
            <RULE_NAME>
              <xsl:value-of select="CUSTOM_TWEAKING_RULES" />
            </RULE_NAME>
          </TWEAKING_RULES>
        </ns1:Insert>
      </xsl:for-each>
