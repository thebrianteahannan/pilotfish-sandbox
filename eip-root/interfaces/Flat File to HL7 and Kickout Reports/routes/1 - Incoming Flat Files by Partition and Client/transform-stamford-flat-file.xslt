...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dtFormatter="xalan://com.pilotfish.eip.gui.mapper.util.DateTimeFormatter" xmlns:pf="http://pilotfishtechnology.com" exclude-result-prefixes="dtFormatter" version="3.1">
  <xsl:template match="/">
    <XCSData>
      <Group key="">
        <Unknown />
        <Header>
          <FacilityName>Stamford Facility</FacilityName>
        </Header>
      </Group>
      <xsl:for-each select="//XCSRecord[H != 'T']">
        <xsl:variable name="AccountNumber" select="EXPORT_FILE_TICKET" />
        <xsl:if test="$AccountNumber != 'Export File/Ticket'">
          <Group key="{$AccountNumber}">
            <PatientDemographics>
              ...
              <absAdmitdate>
                <xsl:if test="string-length(Unknown_Data[28]) != 0">
                  <xsl:value-of select="dtFormatter:format(Unknown_Data[28],'MM/dd/yyyy','yyyyMMdd')" />
                </xsl:if>
              </absAdmitdate>
              <absattendingdocupin />
              <absdischargedate>
                <xsl:if test="string-length(Unknown_Data[29]) != 0">
                  <xsl:value-of select="dtFormatter:format(Unknown_Data[29],'MM/dd/yyyy','yyyyMMdd')" />
                </xsl:if>
              </absdischargedate>
              <admemplname />
              <admzipcode>
              ...
              <absattendingdocmnem />
              <absattendingdocname>
                <xsl:value-of select="Unknown_Data[7]" />
              </absattendingdocname>
              <admemplstreet />
              <admAcctNum>
              ...
              <admbirthdate>
                <xsl:if test="string-length(Unknown_Data[24]) != 0">
                  <xsl:value-of select="dtFormatter:format(Unknown_Data[24],'MMddyyyy','yyyyMMdd')" />
                </xsl:if>
              </admbirthdate>
              <admLocation>S.LOC</admLocation>
              <admpatsex>
              ...
              <admssn>
                <xsl:value-of select="Unknown_Data[30]" />
              </admssn>
              <admname>
              ...
              <admFinClass />
              <AttendDrNPI>
                <xsl:value-of select="Unknown_Data[8]" />
              </AttendDrNPI>
              <absPatientUnit />
              <admpatienttype>
            ...
            <Charge>
              <radExamServDate>
                <xsl:if test="string-length(Unknown_Data[11]) != 0">
                  <xsl:value-of select="dtFormatter:format(Unknown_Data[11],'MMddyyyy','yyyyMMdd')" />
                </xsl:if>
              </radExamServDate>
              <ExamApplication />
              <radExamBillingCode>
              ...
              <!--SET THE CPT TO THE SAME VALUE AS CDM (radExamBillingCode) SO MUE EDITS WILL WORK PROPERLY-->
              <radExamCPT>
                <xsl:value-of select="Unknown_Data[13]" />
              </radExamCPT>
              <examdesc />
              ...
              <ExamHCPCCode />
              <radNumOfTimes>
                <xsl:value-of select="Unknown_Data[15]" />
              </radNumOfTimes>
              <radPatientName>
                <xsl:value-of select="Unknown_Data[1]" />
              </radPatientName>
              ...
              <admGuarName>
                <xsl:choose>
                  <xsl:when test="_600 = 'SYSTEM GENERATED' or string-length(_600) = 0">
                    <!--DEFAULT TO PATIENT NAME IF THERE IS NO GUARANTOR NAME-->
                    <xsl:value-of select="Unknown_Data[1]" />
                  </xsl:when>
                  <xsl:otherwise>
            ...
            <Insurance1>
              ...
              <adminsinsuredrel>
                <xsl:choose>
                  <xsl:when test="contains(Unknown_Data[41],'[')">
                    <xsl:value-of select="substring-before(Unknown_Data[41],' [')" />
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="Unknown_Data[41]" />
                  </xsl:otherwise>
                </xsl:choose>
              </adminsinsuredrel>
              <adminsstreet>
